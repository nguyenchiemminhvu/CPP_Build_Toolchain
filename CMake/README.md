...
```OUTPUT```: Specifies the files generated by the custom command. These files are considered outputs and can be used as dependencies for other targets.

```COMMAND```: Defines the command to be executed. Multiple commands can be specified, and each can have its own arguments.

```MAIN_DEPENDENCY```: Specifies the main dependency file for the command. This is often the primary input file.

```DEPENDS```: Lists files or targets that the command depends on. The custom command will be executed if any of these dependencies change.

```BYPRODUCTS```: Specifies additional files created by the command that are not listed in OUTPUT.

```IMPLICIT_DEPENDS```: Specifies implicit dependencies for the command, useful for languages like C/C++ where the dependency scanner can find additional dependencies.

```WORKING_DIRECTORY```: Sets the working directory where the command will be executed.

```COMMENT```: Provides a comment or message that will be displayed when the command runs.

```DEPFILE```: Specifies a file containing dependencies for the command, often used with compilers that generate dependency information.

```JOB_POOL```: Assigns the command to a specific job pool, controlling parallel execution.

```VERBATIM```: Ensures that command arguments are passed exactly as specified, without modification.

```APPEND```: Appends the command to the existing command list for the specified outputs.

```USES_TERMINAL```: Indicates that the command uses a terminal, which affects how its output is handled.

```COMMAND_EXPAND_LISTS```: Expands lists in command arguments.
