...
The output of Autoconf process is a 'configure' file. The configuration scripts generated by Autoconf can stand on their own, meaning that users can run them without needing to have Autoconf installed. A configuration script might be complicated, but there are only a few things the user needs to specify regarding components, features, and options to be enabled, like: where the header files and libraries are located? Where to install final product? Which optional components to include?...
